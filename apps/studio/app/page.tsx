...
} from "@lacuna-engine/content-loader";
...
export default function StudioPage() {
  ...
  const results = worldIds.map((worldId) =>
    validateWorldPackById(worldId, { contentDir: CONTENT_DIR })
  );
...
function WorldPreview({
  ...
  world: NonNullable<ReturnType<typeof validateWorldPackById>["world"]>;
}) {
  return (
    <div style={previewStyle}>
      ...
      {world.cities.map((city) => (
        <section key={city.id} style={cityStyle}>
          ...
          <dl style={metaGridStyle}>
            <Metric label="State Keys" value={Object.keys(city.initialState).join(", ")} />
            <Metric label="Entry Roles" value={String(city.entryRoles.length)} />
            ...
            <Metric label="Actions" value={String(city.placeholderActions.length)} />
          </dl>
        </section>
      ))}
    </div>
...
const bodyStyle = { margin: 0, color: "#475569", fontSize: 14, lineHeight: 1.6 };
